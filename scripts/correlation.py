  ...
  for run in data:
    ...
    if use_individual_records:
      ...
      for r, e in zip(rewards, entropies):
        data_list.append({"noise_type": noise_type, "reward": r, "entropy": e})
    else:
      ...
      data_list.append({"noise_type": noise_type, "reward": mean_reward, "entropy": mean_entropy})
...
def analyze_anova_and_correlation(use_individual_records=False):
  ...
  for env_id in envs:
    for raw_cfg in ["trpo", "gentrpo"]:
      ...
      # ANOVA/Kruskal-Wallis: Effect of noise type on rewards
      unique_noises = df_noise_reward["noise_type"].unique()
      reward_groups = [df_noise_reward[df_noise_reward["noise_type"] == n]["reward"] for n in unique_noises]
      reward_groups = [g for g in reward_groups if len(g) > 0]  # Filter empty groups

      if len(reward_groups) < 2:
        anova_interpretation = f"Insufficient groups (noise types) for statistical test in {key}."
        post_hoc = None
      else:
        ...
        anova_interpretation = f"{test_name} for noise type effect on rewards: stat={stat:.2f}, p={p_value_test:.2e}. The impact is {significance}."

        # Post-hoc if significant
        post_hoc = None
        if p_value_test < 0.05:
          if use_parametric:
            tukey = pairwise_tukeyhsd(df_noise_reward["reward"], df_noise_reward["noise_type"])
...
if __name__ == "__main__":
  ...
  for key, res in analysis_results.items():
    ...
    print("Statistical Test (Noise Type vs Rewards):")
