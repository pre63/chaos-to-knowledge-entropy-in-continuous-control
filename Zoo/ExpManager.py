...
class ExperimentManager:
  ...
  def setup_experiment(self) -> Optional[Tuple[BaseAlgorithm, Dict[str, Any]]]:
    ...
    hyperparams, self.env_wrapper, self.callbacks, self.vec_env_wrapper = self._preprocess_hyperparams(unprocessed_hyperparams)
  ...
  def learn(self, model: BaseAlgorithm) -> None:
    """
        ...
        """
    print(f"Learning model for {self.n_timesteps} timesteps")
    kwargs: Dict[str, Any] = {}
  ...
  def _save_config(self, saved_hyperparams: Dict[str, Any]) -> None:
    ...
    with open(os.path.join(self.params_path, "args.yml"), "w") as f:
      ordered_args = OrderedDict([(key, args_[key]) for key in sorted(args_.keys())])
  ...
  def _preprocess_action_noise(
    ...
    if hyperparams.get("noise_type") is not None:
      ...
      assert isinstance(env.action_space, spaces.Box), f"Action noise can only be used with Box action space, not {env.action_space}"
  ...
  def is_robotics_env(env_id: str) -> bool:
    return "gym.envs.robotics" in ExperimentManager.entry_point(env_id) or "panda_gym.envs" in ExperimentManager.entry_point(env_id)
    ...
    if not is_vecenv_wrapped(env, VecTransposeImage):
      ...
      if isinstance(env.observation_space, spaces.Dict):
        ...
        for space in env.observation_space.spaces.values():
          wrap_with_vectranspose = wrap_with_vectranspose or (is_image_space(space) and not is_image_space_channels_first(space))  # type: ignore[arg-type]
      else:
        wrap_with_vectranspose = is_image_space(env.observation_space) and not is_image_space_channels_first(env.observation_space)  # type: ignore[arg-type]
  ...
  def _load_pretrained_agent(self, hyperparams: Dict[str, Any], env: VecEnv) -> BaseAlgorithm:
    ...
    if os.path.exists(replay_buffer_path):
      ...
      assert hasattr(model, "load_replay_buffer"), "The current model doesn't have a `load_replay_buffer` to load the replay buffer"
