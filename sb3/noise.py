    ...
    for config in self.noise_configs:
      if config["component"] == "reward":
        ...
        if noise_type == "gaussian":
          noise = np.random.normal(0, entropy_level * self.base_std)
        elif noise_type == "uniform":
          noise = np.random.uniform(-entropy_level * self.base_range, entropy_level * self.base_range)
        elif noise_type == "laplace":
          noise = np.random.laplace(0, entropy_level * self.base_scale)
  ...
  def _add_action_noise(self, action):
    ...
    for config in self.noise_configs:
      if config["component"] == "action":
        ...
        entropy_level = abs(config["entropy_level"])
        if noise_type == "gaussian":
          noise = np.random.normal(0, entropy_level * self.base_std, size=action.shape)
        elif noise_type == "uniform":
          noise = np.random.uniform(-entropy_level * self.base_range, entropy_level * self.base_range, size=action.shape)
        elif noise_type == "laplace":
          noise = np.random.laplace(0, entropy_level * self.base_scale, size=action.shape)
        noisy += noise
